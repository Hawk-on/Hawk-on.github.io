...
const Kontakt: React.FC = () => {
  ...
  // Bruk 'no' som standard fram til me er montert på klienten for å unngå hydration error
  const visSpraak = mounted ? aktivSpraak : 'no';
  const t = omsetjingar[visSpraak].contact;

  return (
    <div className="kontakt-lenker">
      <a href={`mailto:${email}`} className="kontakt-item" id="kontakt-epost" data-umami-event="kontakt-epost">
        <span className="kontakt-item__type">{t.emailLabel}</span>
      ...
      <a href={`tel:${phone.replace(/ /g, '')}`} className="kontakt-item" id="kontakt-tlf" data-umami-event="kontakt-telefon">
        <span className="kontakt-item__type">{t.phoneLabel}</span>
